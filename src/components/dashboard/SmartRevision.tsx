...
export default function SmartRevision({ userId }: { userId: string | null }) {
  ...
  // 🚀 Logic: Find Surahs not read in > 7 days
  const processLogs = (logs: any[]) => {
    const lastReadMap: Record<number, string> = {};

    // 1. Find the latest date for each Surah
    logs.forEach((log) => {
      ...
      if (current > existing) {
        lastReadMap[log.surah_number] = log.created_at;
      }
    });
    ...
    Object.keys(lastReadMap).forEach((surahStr) => {
      ...
      // ⚠️ Threshold: If older than 7 days, add to list
      if (diffDays > 7) {
        needsReview.push({ surah, daysAgo: diffDays, lastRead: lastReadMap[surah] });
      }
    });

    // 3. Sort by "Most Neglected" (High days ago first)
    return needsReview.sort((a, b) => b.daysAgo - a.daysAgo).slice(0, 3); // Show top 3
  };
    ...
    async function fetchHistory() {
      ...
      if (userId === 'guest') {
        ...
        const { data } = await supabase
          .from('progress_logs')
          .select('surah_number, created_at')
          .eq('user_id', userId);
        logs = data || [];
      }
  ...
  return (
    <Card className="border-white/5 bg-[#0a0a0a] overflow-hidden relative group p-6">
      {/* Background Decor */}
      ...
      <div className="relative z-10 space-y-6">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">
            Surahs you haven't reviewed in 7+ days.
          </p>
        </div>
        ...
        {revisionList.length > 0 && (
          <div className="pt-2">
            <Button 
              onClick={() => router.push('/recite')}
              className="w-full bg-amber-600/20 hover:bg-amber-600/30 text-amber-500 border border-amber-500/50 font-bold h-10 rounded-xl text-xs uppercase tracking-wider gap-2"
            >
              Start Revision <RefreshCw className="h-3 w-3" />
            </Button>
          </div>
        )}
      </div>
